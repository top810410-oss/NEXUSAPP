...
//            }
        ...
        if($ret['value'] == 1){
          $qiangUserArr =  DB_QiangHongBao::select();
          $rand = rand(0,count($qiangUserArr)-1);
         //  return $rand.json_encode($qiangUserArr);
        // echo($rand);
        $quser_id = $qiangUserArr[$rand]['user_id'];
        echo($quser_id);
        self::autoGetHongBao(['rid' => $hongbao->id,'user_id' => $quser_id]);
        } 
     ...
     */
    public static function autoGetHongBao(array $params)
    {
        //1.判断包存不存在
        //2.判断用户是否在会话之中
        //3.红包是否被抢完
...
//        Db::commit();
        //发送系统通知
        $res_3 = DB_QiangHongBao::where(['user_id' => $params['user_id']])->setInc('qiang_money', floatval($big_red_amount));
        $content = [
