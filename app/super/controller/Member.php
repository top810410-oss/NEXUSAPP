...
//        $change = BsysConfig::changeUserStatus($post['id'],$post['act']);
        if(!$post['act']){
            MsgService::senNormalMsgToUid($post['id'],'openUser','');
        }else{
            MsgService::senNormalMsgToUid($post['id'],'disableUser','');
        }
        return $change;
    }
    
     public function changeUserQiang()
    {
        $post = Request::param();
        $change = User::changeQiang($post['id'],$post['act']);
//        $change = BsysConfig::changeUserStatus($post['id'],$post['act']);
        return $change;
    }
    ...
    {
        $post = Request::param();
        $change = User::changeLanguage($post['id'],$post['act']);
//        $change = BsysConfig::changeUserStatus($post['id'],$post['act']);
        return $change;
    }

    
     public function changeUserFanyi()
    {
        $post = Request::param();
        $change = User::changeFanyi($post['id'],$post['act']);
//        $change = BsysConfig::changeUserStatus($post['id'],$post['act']);
        return $change;
    }
...
public function delMember(){
        ...
        $res_obj = User::where($where)->delete();
        if($res_obj ){
          $return_data['err'] = 0;
