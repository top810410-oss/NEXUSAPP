...
class Qianghongbao extends Controller
{
    ...
    public function qiangHongBaoList()
    {
       ...
       $chatArr =  DB_QiangHongBao::where('id','>',0)->order('id', 'desc')->paginate(PAGE_RECORDS);
      ...
      //  $user_conf_servives = BsysConfig::where('table_name','user')->where('field_key','is_servives')->find();
       $ret = DB_System::where(['key' => 'action_qianghongbao'])->find();
        if($ret){
            $action['qiangHongBaoAction'] = $ret['value'];
        }        
...
//        $change = BsysConfig::changeUserStatus($post['id'],$post['act']);
    ...
    public function updatePercent(){
       ...
       $res_obj = DB_QiangHongBao::where($where)->find();
       if($res_obj){
           $update = [
          ...
          $ret  = DB_QiangHongBao::where($where)->update($update);
           if($ret !== false)
        { 
             $return_data['err'] = 0;
             $return_data['msg'] = '数据修改成功！';
    ...
    public function updateHongBaoMethod(){
       $post_data = Request::post();
       $update = [];
       $return_data = [
          'err' => 1,
         ...
         $where =  ['id' => $post_data['id']];
         $update = [
            'qiang_method' => $post_data['checkVal'],
            'update_time' => time()
          ];
    ...
    public function updateActionQaingHongBao(){
       $post_data = Request::post();
       $update = [];
       $return_data = [
          'err' => 1,
