...
class Announcement extends Controller
{
    ...
    public function addAnnouncement()
    {
        ...
        if (Request::isAjax()) {
           // if(empty($params['small_pic']))
           // return json(JsonDataService::fail('请上传图片'));
            
            if (!$params['id']) {
                $ret = MAnnouncement::create($params);
            } else {
                $artice = MAnnouncement::find($params['id']);
        ...
        if($params['id']){
            $params = MAnnouncement::find($params['id']);
        }
