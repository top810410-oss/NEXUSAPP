...
//         $post_data['content'] = json_decode($post_data['content']);
              ...
              $user_data_obj = User::where(['id' => $value['user_id']])->find();
    ...
    /** 获得与客服的会话id */
    public static function getListIdByCustomer($customer_id)
    {
        $return_data = [
            'err' => 1,
            'msg' => 'fail',
        ...
        $chat_user_ids = [USER_ID, ($customer_id * 1)];
        sort($chat_user_ids);
        $chat_user_ids = json_encode($chat_user_ids);
