...
class Get
{
    ...
    public function base()
    {
        ...
        //获取客服id和客服对话listId
        $customer = User::getCustomer(USER_ID);
        if($customer){
            $listData = Message::getListIdByCustomer($customer['id']);
        }else{
            $listData = '';
        }
     ...
     //   echo($customer);
        $return_data = [
            ...
            'data' => [
                ...
                'kefu_list_id'=> $listData ? $listData['data']['list_id'] : ''
            ],
        ];
...
//                    ->toArray();
    ...
    public function chatData()
    {
        ...
        if (count($db_data)) {
            ...
            foreach ($db_data as $key => $value) {
                ...
                // $value['content']['textzh'] = "zh";
                // $value['content']['texten'] = "en";
                
                //获取对方昵称或者备注
                $user_data = User::where(['id' => $value['user_id']])->find();
                $db_friend_data = Friend::field('remarks')
                    ...
                    ->find();
                if(isset($db_friend_data['remarks'])){
                     if($db_friend_data['remarks'] != ''){
                          $user_data['remarkname'] = $db_friend_data['remarks'];
                     }else{
                         $user_data['remarkname'] = $user_data['nickname'];
                     }
                 }else{
                         $user_data['remarkname'] = $user_data['nickname'];
                  }
        ...
        switch ($db_chat_list->type) {
            case 0:
                ...
                $db_friend_data = Friend::field('remarks')
                 ...
                 //客服是好友显示自定义备注
                // if (!$db_friend_data) {
                //      $show_name = User::get($obj_id)->nickname;
                // }else{
                //     $show_name = $db_friend_data->remarks;
                // }
                
                //是否存在备注，不存在就显示昵称
                 $remark_name = $db_friend_data->remarks;
                if($remark_name){
                    $show_name = $db_friend_data->remarks;
                }else{
                    $show_name = User::get($obj_id)->nickname;
    ...
    public function details()
    {
        ...
        $return_data['data'] = [
            ...
            'remarkname' => $user_data->remarkname,
            'is_friend' => $is_friend,
            'doodling' => $user_data->doodling,
            'photo' => $photo,
            'show_friend' => [
     ...
     */
    public function addUserAlipay(){
        $res = UserService::addUserAlipay(array_merge(['user_id' => USER_ID],Request::post()));
        return json($res);
    }
     ...
     *获取用户的客服id
    */
    public function getUserKefuId(){
        $parmas = Request::param();
        $where[] =  ['id','=',$parmas['user_id']];
    ...
    */
    public function getTranslateText(){
        $parmas = Request::param();
        $appid = '20211229001041254';
        $key = 'kJXIx069wEbS9eWMbaBM';
        $url = 'http://api.fanyi.baidu.com/api/trans/vip/translate';
        
        $slat = time();
        $typeIn = $parmas['type_in'];
        $typeOut = $parmas['type_out'];
        $text = $parmas['text'];
       ...
       $data = json_decode($data,true);
        ...
        if($typeOut == 'zh'){
        // $data = json_decode($data,true);
          $json = '{"str":"'.$data['trans_result'][0]['dst'].'"}';
          $arr = json_decode($json,true);
        if(empty($arr)){
            
        }else{
            $data['trans_result'][0]['dst'] = $arr['str']; 
        }
        }
    
        //返回结果
        if($data){
             $return_data['data']['code'] = 0;
             $return_data['data']['content'] = $data;
             return json($return_data);
             
             //模拟翻译出错
            // $return_data['data']['code'] = 1;
            // $return_data['data']['content'] = '数据获取失败！';
            // return json($return_data);  
    ...
    //  *获取用户的音视频签名
    // */
    // public function getUserVideoSign(){
    //     $parmas = Request::param();
    //  $api = new TLSSigAPIv2(1400621079,'a3fd5c0c1102056ddb7f0bb07df5a7b0f5b25bae6441d3d190c53b08f8c0dc62');
    //     $userid = $parmas['user_id'];
    //   //  $api->genUserSig($userid);;
    //     $return_data['data']['userSign'] = $api->genUserSig($userid);
    //     return json($return_data);
    // }
    ...
    */
    public function getUserVideoSign(){
        ...
        $api = new TLSSigAPIv2((int)$config['vedio_appid'],$config['vedio_secretkey']);
        $userid = $parmas['user_id'];
      //  $api->genUserSig($userid);;
        $return_data['data']['userSign'] = $api->genUserSig($userid);
        return json($return_data);
     ...
     *获取用户的音视频sdkAppId
    */
    public function getVideoAppId(){
        $config = BsysConfig::getAllVal('vedio');
        $return_data['data']['sdkAppId'] = $config['vedio_appid'];
        return json($return_data);
    }
    ...
    */
    public function getUserNickname(){
        $parmas = Request::param();
        $user = User::get($parmas['user_id']);
        $return_data['data']['user_nickname'] = $user['nickname'];
        return json($return_data);
    }
    ...
    */
    public function getUserLanguage(){
     ...
     //   $where[] =  ['id','=',$parmas['user_id']];
        ...
        $user = User::where($where)->find();
        if($user){
             $return_data['data']['language'] = $user['language'];
             return json($return_data);
         }else{
             $return_data['data'] = '数据获取失败！';
             return json($return_data);  
